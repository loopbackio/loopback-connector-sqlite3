...
SQLite3.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  for (var p in params) {
    ...
    if (params[p] instanceof Error && params[p].message.match(/SQLITE3:/))
      return callback(params[p]);
  }
...
SQLite3.prototype.createTable = function(model, cb) {
  ...
  stmts.concat(self._buildColumnIndexes(tableName, model));

  async.each(stmts, self.executeSQL.bind(self), cb);
};
...
SQLite3.prototype._buildColumnIndexes = function(tableName, model) {
  var properties = this.getModelDefinition(model).properties;
  var indexes = [];
  for (var propertyName in properties) {
    if (!properties.hasOwnProperty(propertyName)) continue;
    ...
    if (property.index)
      indexes.push(fmt('CREATE INDEX IF NOT EXIST %s_%s ON %s (%s)',
        tableName, propertyName, tableName, propertyName
      ));
  }
  return indexes;
};
...
SQLite3.prototype.toColumnValue = function(property, value) {
  ...
  switch (property.type.name) {
    case 'Number':
      if (isNaN(value)) return Error('SQLITE3: Invalid number: ' + value);
      return Number(value);
    case 'Boolean':
    ...
    case 'JSON':
      try {
        ...
        return Error('SQLITE3: Invalid JSON: ' + value);
      }
      return String(value);
    case 'Date':
      if (!isNaN(value))
      ...
      return Error('SQLITE3: Invalid date: ' + value);
    case 'Array':
    default:
      return value;
  }
